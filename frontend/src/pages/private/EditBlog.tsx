...
const EditBlog = () => {
    ...
    const handleSubmit = async (published: boolean) => {
        try {
            ...
            const editedBlog = await editBlog({ title, content, category, published, coverImage, blogId: blogId || '' })
            navigate(`/${user.username}/${editedBlog.id}`)
        } catch (error: any) {
    ...
    return (
        <div>
            {
                loading ?
                    <Container className='h-[95vh] flex flex-col justify-between py-4 lg:py-6'>
                        <Skeleton className='h-[8%] max-w-3xl' />
                        <Skeleton className='h-[30%] max-w-3xl' />
                        <Skeleton className='h-[8%] max-w-3xl' />
                        <Skeleton className='h-[40%]' />
                        <div className="flex justify-center items-center gap-1.5  lg:gap-3 flex-wrap h-[5%]">
                            <Skeleton className='h-full w-32 lg:w-40' />
                            <Skeleton className='h-full w-32 lg:w-40' />
                        </div>
                    </Container>
                    :
                        ...
                        data ?
                            <Container className="text-[#1E1E21] py-4 lg:py-6">
                                ...
                                <div className="mb-4 lg:mb-6 max-w-3xl">
                                    ...
                                    <div className="rounded-[10px] border-[rgba(30,30,33,0.8)] focus:outline-none focus:border-[rgba(30,30,33,1)] border-solid border-[1px] w-full pb-1 pl-1">
                                        <input type="file" onChange={(e) => {
                                            setCoverImage(e.target.files && e.target.files[0])
                                        }} id="cover-image" className="px-3 lg:px-5 py-2.5 lg:py-4 " />
                                        {
                                            coverImage &&
                                            <img src={coverImage} className="max-h-[300px] block object-contain" />
                                        }
                                    </div>
